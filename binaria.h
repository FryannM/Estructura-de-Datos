...
void insertarDato(int n){
	...
	//Algoritmo de busqueda;
	 ...
	 cout<<"\nDigite el numero a buscar en el array:";
	 cin>> dato;
	  while(inf <=sup){
	  	mitad = (inf+sup)/2;
		...
		if(band == 'F'){
		cout<<"\nEl numero "<<dato<<" buscar no existe en el arreglo"<<endl;
	}
	else if(band == 'V'){
		cout<<"\nEl numero  "<<dato<<"  a sido encontrado en la pos: "<<mitad<<endl;
	}
	
	gettimeofday(&t_fin, NULL);
	secs = Ttimeval_diff(&t_fin, &t_ini);
    printf("\nTiempo: %.16g milisigundos\n", secs * 1000.0);
